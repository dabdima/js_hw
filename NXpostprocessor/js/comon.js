...
var turnDataBack = function() {
	TN = 0, TNlast = 0, iForN=0;
	 fromExistance = 0;
	 Fpast = 5000, F = 0, Xpast = 0, Zpast = 0, X = 0, Z = 0, GPathModePast = "G1", GPathMode = "G1", GFeedModePast = "G94"; GFeedMode = "G94";
	 G1Factor = 1, ArcGFactor = 0, G94Factor = 1, G95Factor = 0, FFactor = ""; //FFactor - cutting feed mode G94 or G95
...
function getArcDirection(str){
	...
	if ((Number(res[5]) != -1) && (Number(res[5]) != 1)) console.log("function getArcDirection error: res[5] = "+res[5]);
}
...
function getResult() {
	...
	for (i = 0; i < clDataArray.length; i++) {
		...
		if ((getXXXX(clDataArray[i]) == "Инфо") && (getXXXX(clDataArray[i+1]) == "Дата") &&
			...
			if (TN == 0) {
				if (($(".active-cnc-set div .cuselText").text() == "SBL-500")||($(".active-cnc-set div .cuselText").text() == "SE-820")) {
					CNC[i][0] = "<span style='font-style: italic;'>" //needed for closure "</span>" for color mode
			...
			if (re.exec(toolStrParams.passName) === null) { continue } else {
				...
				if (mPrev != toolStrParams.m) {
					//if (($(".active-cnc-set div .cuselText").text() == "SBL-500")||($(".active-cnc-set div .cuselText").text() == "Lynx300M")||($(".active-cnc-set div .cuselText").text() == "SE-820")) {
						...
						iForN+= iDelta;
						CNC[i][0] = nBlock() +toolStrParams.range+" S"+toolStrParams.s+" M"+toolStrParams.m;
					}
		...
		if (getXXXX(clDataArray[i]) == "TURR") { // get tool number & generate starting block
			TN = getToolNumber(clDataArray[i]);
			toolStrParams.t = TN;
			if (TN != TNlast) {
					var blockAddress;
				...
				if ($(".active-cnc-set div .cuselText").text() == "SBL-500") {
					CNC[i-7][0] = blockAddress + startBlockNo +" T"+TN+" D1"+"\n"+nBlock()+"G18\n"+nBlock()+"G64 G90 G94\n"+nBlock()+"G97" +" S"+toolStrParams.s+" M"+toolStrParams.m;
				}
...
/* 	setTimeout(function() {
	...
	$("*:not(#about-information, #about)").on('click', function(event) {  
		if ((a1===0)&&(b1===0)&& (!($("#about-information").hasClass("moveright")))) {
			...
			//$("#about-information").css("right", "-300px !important;");
			
			//a1+= 1;
			
		} //else {$("#about-information").addClass("moveright") ;
		//}
		console.log("click", $("#about-information").hasClass("moveright") );
	});
	
	/* $("*:not(#about)").on('click', function(event) {  
		if ((!($("#about-information").hasClass("passive-cnc-set")))&&(a1 != 0)) {
			$("#about-information").toggleClass("passive-cnc-set");
